...
def new(location):
    store = C.zcertstore_new(location)
    def destroy(c):
        C.zcertstore_destroy(ptop('zcertstore_t', c))
    return ffi.gc(store, destroy)

lookup = C.zcertstore_lookup
insert = C.zcertstore_insert
dump = C.zcertstore_dump
test = C.zcertstore_test
